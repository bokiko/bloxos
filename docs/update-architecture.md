...
`preflight()` read-only deployment evidence; `stage(manifest, release_dir)` validate/extract artifacts or pull images before downtime; `quiesce()` stop public proxy THEN hub/dashboard; `backup()` persist original data+artifacts+config while stopped; `install()` switch only selected app artifacts; `start_candidate()` start hub/dashboard and original proxy with maintenance marker; `identities()` returns direct hub/dashboard metadata; `rollback()` stops proxy and components, restores matching snapshot/artifacts/config and restarts original components with the proxy still closed; `resume_original()` recovers a failed backup before install without restoring absent snapshots; `finalize()` reopens the proxy only after the terminal phase is durable. Adapter journals all original paths/image references/units needed for idempotent rollback in transaction directory BEFORE mutations.

Compose records the original restart policies and disables automatic restart on the three selected containers before downtime. Candidate and rollback configurations retain `restart: no` until finalization restores the original policies. A Docker restart therefore cannot reopen a legacy hub ahead of recovery. A crash during finalization retries finalization, never restores the already-committed snapshot again.
