...
func (s *Server) handleCreateToken(c echo.Context) error {
	...
	if _, err := s.db.Exec(
		...
		tokenHash, tokenExpiryText(expiresAt), httpBase, caSHA256); err != nil {
...
func (s *Server) handleWindowsReenrollment(c echo.Context) error {
	...
	if _, err := s.db.Exec(
		...
		tokenHash, tokenExpiryText(expiresAt), machineID,
	); err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": err.Error()})
	}
...
# Minimal root-operated hosts and containers often have no sudo; only ask
...
func publicAndWebsocketBase() (string, string) {
	...
	// Normalize once, here, so every consumer (paste block, install.sh URLs,
	// the agent's BLOXOS_HUB + "/ws/agent", the mint-time binding and its
	// drift check) sees the same origin. A trailing slash otherwise produced
	// "//install.sh" paths that the router does not collapse.
...
func checkTokenExpiry(expiresAt string) error {
	expTime, err := parseTokenExpiry(expiresAt)
	if err != nil {
		return err
	}
...
// tokenExpiryText is how expiry is stored from now on: UTC RFC3339 text.
// It round-trips through the driver unchanged, parses here, and is a form
// SQLite's own date functions understand, unlike the driver's rendering of
// a time.Time, which carries a local offset and zone name.
func tokenExpiryText(t time.Time) string {
	return t.UTC().Format(time.RFC3339)
}

// parseTokenExpiry accepts every form tokens.expires_at has ever held when
// read back through the driver: the UTC text written now, the RFC3339 with
// local offset the driver produces for rows older hubs wrote as time.Time,
// and the plain "2006-01-02 15:04:05" layout (read as UTC).
func parseTokenExpiry(expiresAt string) (time.Time, error) {
	...
	// The driver's raw rendering of a time.Time is "date time offset zone".
	// The numeric offset is authoritative; the zone name can be anything
	// (including forms Go's MST token rejects, such as a repeated "-0400"),
	// so drop it and parse the remaining three fields.
	if fields := strings.Fields(expiresAt); len(fields) == 4 {
		if t, err := time.Parse("2006-01-02 15:04:05.999999999 -0700", strings.Join(fields[:3], " ")); err == nil {
			return t, nil
		}
	}
...
// deleteExpiredTokens removes tokens whose expiry is before now, deciding in
// Go with parseTokenExpiry so legacy local-offset rows are judged by their
// actual instant. Unparseable rows are left alone and logged rather than
// guessed at.
func (s *Server) deleteExpiredTokens(now time.Time) (int64, error) {
	rows, err := s.db.Query(`SELECT token_hash, expires_at FROM tokens`)
	if err != nil {
		return 0, err
	}
	var expired []string
	for rows.Next() {
		...
		t, err := parseTokenExpiry(expiresAt)
		if err != nil {
			log.Printf("cleanup: token with unparseable expiry %q left in place", expiresAt)
			continue
		}
		if now.After(t) {
	...
	for _, hash := range expired {
		res, err := s.db.Exec(`DELETE FROM tokens WHERE token_hash = ?`, hash)
		if err != nil {
			return total, err
		}
...
func (s *Server) generateFirstRunToken() {
	...
	_, err := s.db.Exec(`INSERT INTO tokens (token_hash, expires_at) VALUES (?, ?)`, tokenHash, tokenExpiryText(expiresAt))
	if err != nil {
