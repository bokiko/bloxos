...
// fleetPowerUnverifiedSystemSources are backends whose readings are real
// measurements of SOMETHING, but not demonstrably of this machine.
//
// Both were generated by older agents and both still arrive from them and from
// journal replay, so removing the agent-side backends does not remove the rows.
//
//   - battery discharge is the machine's draw only while the machine runs on
//     that pack alone; on AC, or with one pack charging while another
//     discharges, the packs supply part of the load and the recorded number is
//     a subset presented as a total.
//   - a shunt or ACPI meter reports whatever rail it is wired across. The chip
//     name says something measures, never what.
//
// They are excluded from SYSTEM totals and counted with the unknown machines,
// captioned as source or scope unverified — "unrecognised" would be wrong
// about a chip this hub recognises perfectly well.
...
func fleetPowerClassify(domain, source string) string {
	...
	// Scope, in the one domain where it is a claim about the whole machine.
	// A shunt's reading is a measurement; "this machine draws that" is an
	// inference from a chip name, and the agent no longer makes it. Rows that
	// already carry it are not rewritten — they are simply not added up.
	...
	// hwmon outside the system domain still names the chip it found, so the
	// identifier is open-ended and cannot be enumerated. There it classifies
	// the reading as a MEASUREMENT and says nothing about scope, which the
	// agent's domain assignment carries.
...
// fleetPowerKindFor is fleetPowerClassify plus what the SAMPLES themselves say.
//
// One signature has to be caught here because it can no longer be caught at
// the source: a RAPL group whose every counter was frozen used to be reported
// as a valid 0.0 W reading, so a running CPU appeared to draw nothing. The
// agent now refuses that, but the rows are already stored and older agents
// still send them.
//
// The test is mean AND peak both exactly zero over a window that had samples.
//
// This does NOT prove the row was fabricated, and it is not treated as proof.
// Samples and expected cannot separate a frozen counter from a window of
// genuinely zero energy, and that inability is precisely the reason to
// withhold: an all-zero RAPL window is unverified, so it is not summed, and
// the stored row is left exactly as it is.
//
// It is deliberately NOT a blanket "greater than zero" filter. A GPU parked at
// 0 W and a BMC reporting an active zero are real readings and must survive,
// which is why this is scoped to RAPL sources and to the unlabelled CPU and
// DRAM domains the protocol defines as RAPL. A positive peak with a mean that
// rounds to zero is not this case either: the counter moved.
...
// domain/source pair is the frozen-counter signature rather than a reading.
//
// Unlabelled CPU and DRAM are included because the protocol defines them as a
// RAPL package sum and RAPL dram: the agents that emit no source label predate
// labelling entirely, and they are the ones most likely to carry the defect.
//
// Unlabelled SYSTEM is deliberately NOT included. The protocol does not pin it
// to RAPL, so an all-zero unlabelled system window cannot be attributed to a
// frozen energy counter without guessing which backend produced it.
func fleetPowerFrozenCounterDomain(domain, source string) bool {
	...
	case powerhistory.SourceRAPLPsys, powerhistory.SourceRAPLPackage, powerhistory.SourceRAPLDRAM:
		return true
	case "":
		return domain == powerhistory.DomainCPU || domain == powerhistory.DomainDRAM
	}
	return false
}
...
type FleetPowerBucket struct {
	...
	// hub will not add up: an unrecognised backend, or a recognised one whose
	// SCOPE it cannot vouch for, or a RAPL window whose counters never moved.
	// Their watts are deliberately NOT summed into either series; the count
