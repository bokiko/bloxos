...
- Done:
  ...
  - [x] Phase 10: branding + multi-theme personalization (2026-04-27)
    - Backend: new `branding_config` single-row table (title/subtitle/logo/favicon as PNG bytes in SQLite — `bloxos.db` stays a self-contained backup); new `theme_name` + `theme_mode` columns on `users`. Public read endpoints (`GET /api/branding`, `/api/branding/logo`, `/api/branding/favicon`) so the login page can render branding pre-auth. Admin write endpoints (`PATCH /api/branding`, `POST /api/branding/{logo,favicon}`, `DELETE /api/branding/:kind`) gated on a new `branding.admin` scope (admins only). Per-user endpoints (`GET/PATCH /api/me/theme`, scope `auth.self`). PNG validation via magic bytes + `image/png` decode; SHA-based cache-busting (`?v=<sha>`).
    - Five curated themes in `dashboard/src/app/globals.css` between the existing `:root`/`.dark` blocks and `@theme inline`: BloxOS (default), Solarized (light + dark), Dracula (dark only), Nord (light + dark), Tokyo Night (dark only). Selectors `.theme-<name>` on `<html>`; light/dark still controlled by `.dark` class. Dark-only themes force dark even when user picks light.
    - `ThemeContext` rewritten with named-theme API (`themeName` + `themeMode` + derived `resolvedMode`), localStorage hydration, post-login `/api/me/theme` server sync. `resolvedMode` derived via `useMemo` (was `setState` in the playbook — refactored to satisfy React 19's set-state-in-effect rule). Anti-FOUC inline `themeBootstrapScript` in `layout.tsx` extended to the 5-theme dark-only-fallback logic so first paint is correct.
    - New `BrandingContext` fetches branding on mount (no auth), applies title to `document.title` and updates `<link rel="icon">` href on favicon SHA change. Layout provider order: `AuthProvider > ThemeProvider > BrandingProvider > …` (deviated from playbook because the new ThemeProvider needs `useAuth()` for server sync).
    - New `BrandedHeader` component used by both `app/page.tsx` and `app/login/page.tsx`. Three branches: custom logo (with optional title), title-only, or default `Blox`/`OS` wordmark fallback.
    - New `/settings` route with Theme + Branding tabs (Branding tab admin-gated). `ThemeSettings` is a Mode picker (Light/Dark/System) + Theme grid of 5 preview cards with "Dark only" tag where applicable. `BrandingSettings` has a title/subtitle text form, logo uploader (PNG ≤500KB), favicon uploader (PNG ≤100KB) with toast feedback.
    - User menu dropdown gained a 5-tile gradient theme switcher row (inside `<DropdownMenuGroup>` per Base UI #31) and a "Settings" entry. `Cmd+K` palette gained one "Settings" navigate entry under value `"settings preferences theme branding"`.
    - Backwards compatible: empty branding falls back to "BloxOS" defaults; users with no theme prefs get the existing light/dark/system behaviour.
    - Verified: hub tests 19s green, hub vet clean, dashboard `pnpm lint` 0/0, dashboard `pnpm build` exit 0 with `/settings` prerendered.
  - [x] Phase 11: workflow personalization (2026-04-27)
    - Backend: 4 migrations — adds `display_name`/`avatar_*`/`density`/`default_view`/`default_sort` to `users`; new `user_pinned_machines` (PK user_id+machine_id, ON DELETE CASCADE both ways); new `user_saved_filters` (UUID id, max 20 per user, JSON filter blob); `branding_config.welcome_message` column. SQLite foreign keys pragma was missing — now enabled in production DSN (`hub/main.go`) AND `PRAGMA foreign_keys = ON` in test setup helpers, so cascades on user/machine delete are load-bearing.
    - 10 new endpoints under `scopeAuthSelf` in `hub/preferences.go`: `GET/PATCH /api/me/preferences` (single-call load + scalar update with regex-validated display name `^[\p{L}\p{N} '._-]*$`, density/view/sort enum validation); `POST/DELETE /api/me/avatar` + `GET /api/users/:user_id/avatar` (PNG ≤200KB, magic-bytes + `png.Decode` validation, SHA-based cache-busting); `POST/DELETE /api/me/pinned/:machine_id` (machine-existence check before INSERT OR IGNORE); `GET/POST/DELETE /api/me/filters` (max 20 per user, name ≤60, filter JSON object validation).
    - `BrandingConfig` extended with `WelcomeMessage` (≤500 chars, COALESCE on read, dynamic UPDATE branch on PATCH).
    - Frontend: new `PreferencesContext` with optimistic UI (pin/unpin/save-filter revert on failure, scalar updates apply locally before PATCH), localStorage hydration via lazy `useState` initializer, post-auth `GET /api/me/preferences` reconciliation, JWT user_id extracted from `bloxos_token` claim for avatar URL construction. Effect applies `density-comfortable` / `density-compact` class to `<html>`.
    - Density CSS in `globals.css`: `--card-padding`, `--grid-gap`, `--grid-min-col`, `--card-text-scale` per density mode. Comfortable matches Phase 3 defaults; compact tightens by ~25% padding / 38% gap and narrows column min from 300px to 260px.
    - `MachineCard`: pin star (Star icon, amber filled when pinned, low-opacity muted when not) as first action in the card-header action group; outer card padding swapped from hardcoded `p-4` to `[padding:var(--card-padding)]`. PHASE11-NOTE flagged: every card subscribes to `pinned_machines`, so pinning one re-renders all — acceptable for v1 (≤50 cards), can be hoisted to a `Set` later if needed.
    - `app/page.tsx`: hydrate `viewMode`/`sortBy` from preferences via lazy `useState`; `changeView` / `changeSort` wrappers persist via `updateScalar`; `filteredMachines` does a stable two-phase sort that floats pinned machines to the top regardless of sort order; grid container reads `var(--grid-min-col)` and `var(--grid-gap)`. SaveFilterButton appears when filters are active; SavedFiltersDropdown appears when `saved_filters.length > 0`.
    - New `Avatar` component: uploaded image OR auto-generated initials with `hsl(<hash>, 50%, 45%)` background. Hash is stable across renders so each user gets a consistent generated colour.
    - New `/settings` tabs: Profile (display name + avatar uploader, 200KB PNG cap), Preferences (density / default view / default sort / pinned machines list / saved filters list). Theme + Branding tabs from Phase 10 unchanged. Default tab is now Profile.
    - `UserMenu`: trigger replaced from generic User icon with `<Avatar size={28}>`; dropdown header gained a 32px Avatar + display name + capitalised role row. Phase 10's theme tile row + Settings entry preserved (still inside `DropdownMenuGroup` per Base UI #31).
    - Login page renders `branding.welcome_message` (with `whitespace-pre-line`) below the `BrandedHeader`. `BrandingSettings` extended with a 500-char `<textarea>` wired into the existing PATCH body.
    - Verified: hub tests 19.25s green, hub vet clean, dashboard `pnpm lint` 0/0, dashboard `pnpm build` exit 0, all 9 routes prerendered including `/settings`.
  - [x] Phase 12: operator quality-of-life (2026-04-27)
    - Backend: 2 migrations — `machines.notes TEXT NOT NULL DEFAULT ''` and `metrics.cpu_temp_c REAL` (nullable). New endpoints `GET /api/machines/:id/notes` (`scopeFleetRead`) + `PUT /api/machines/:id/notes` (`scopeFleetMetadata`, 10000-char cap). `handleGetMachine` extended to include `notes`. `handleMetricsHistory` SELECT + Point struct extended with `cpu_temp_c`. New `nullableFloat` helper inserts SQL NULL when value is 0 so old agents don't pollute the metrics table with phantom 0°C readings.
    - Agent: new `collectCPUTempC()` declared per platform via build tags. Linux reads `/sys/class/thermal/*` preferring zones whose `type` contains `pkg_temp`/`coretemp`, falling back to first non-pkg zone, with 0/120°C plausibility filter. Windows queries `MSAcpi_ThermalZoneTemperature` from `root\WMI` namespace, converts tenths-of-Kelvin to Celsius, same plausibility filter. Both return 0 when unavailable so `omitempty` drops the field for older / VM hosts.
    - `Metrics` struct gained `CPUTempC float64 \`json:"cpu_temp_c,omitempty"\``. Metrics ingestion wires it through to the `metrics` table and the SSE broadcast.
    - Frontend: new `MachineNotes` component on the detail page (Notes tab, sticky-note icon, after Overview). Plain-text textarea, character counter, 10000-char cap, URLs auto-linkified to `target="_blank"` via a regex pass in the read view. Edit affordance gated on `hasScope("fleet.metadata")`. Avoids React 19 set-state-in-effect via parent `key={machine.notes ?? ""}` remount — internal state only reacts to user input + PUT success.
    - `MachineCard`: new additive thermal row above the existing GPU row, showing CPU + GPU temps in one quick-scan line. Colour thresholds: <60°C default, 60-80°C amber, ≥80°C red. Picked additive over replace-GPU-temp to avoid layout regressions on existing cards (PHASE12-NOTE in source).
...
- Remaining:
  ...
  - [ ] Decide what to do with `Ai-05` and `dont-know` (both offline WSL machines, `dont-know` has unset hostname) — delete via dashboard, or have their operator re-enrol with proper hostnames.
  - [ ] Implement the unified Add Machine chooser (Linux / Windows / Synology / Proxmox cards) per the saved plan at `thoughts/shared/plans/2026-04-27-unified-add-machine-chooser.md`. ~9h work, no hub API changes required. User said "save it until I tell you when to implement it." Phase 12's two-tab modal is a partial step in this direction.
